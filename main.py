...
def main():
    ...
    fleet_by_class, ask_by_class = {}, {}
    for cyc in final["selected_cycles"]:
        fleet_by_class[cyc.aircraft_class] = fleet_by_class.get(cyc.aircraft_class, 0) + 1
        ask_by_class[cyc.aircraft_class] = ask_by_class.get(cyc.aircraft_class, 0.0) + \
            cyc.total_distance_km * specs[cyc.aircraft_class].seat_capacity

    actual_pax_km = sum(f.pax * f.distance_km for cyc in final["selected_cycles"] for f in cyc.flights)

    cost_model = CostModel(fleet_by_class=fleet_by_class, specs=specs, ask_by_class=ask_by_class)
    breakeven = BreakEvenAnalyzer.solve_ticket_price(cost_model, actual_pax_km)
