...
class GreedyExactCoverSolver(ExactCoverSolver):
    def solve(self, candidate_cycles: list, required_flight_ids: set) -> list:
        ...
        selected = []
        ordered = sorted(candidate_cycles, key=lambda c: -len(c.required_flight_ids))

        for cycle in ordered:
            if not remaining:
                break
            # FIX 1: match against required_flight_ids, not flight_ids, so cycles
            # that were closed with a repositioning leg remain eligible.
            fids = cycle.required_flight_ids
