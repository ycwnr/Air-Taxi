...
def evaluate(indices, ordered_vertiport_ids, required_flights, specs, dist_matrix,
    ...
    if use_cache and cache_key in _pool_cache:
        ...
        pool = generate_cycle_pool(required_flights, specs, station_set, dist_matrix,
                                    pool_size=pool_size)
        if use_cache:
    ...
    fitness = fleet_size + PENALTY_PER_UNCOVERED_FLIGHT * n_uncovered
    return {
        "fitness": fitness, "fleet_size": fleet_size, "n_uncovered": n_uncovered,
        "station_set": station_set, "selected_cycles": selected,
...
def __init__(self, ordered_vertiport_ids, required_flights, specs, dist_matrix=None, **kwargs):
    self.specs = specs
